...
df['is_anomaly'] = rf_model.predict(features)

# Use predicted probabilities to derive anomaly scores
df['anomaly_score'] = rf_model.predict_proba(features)[:, 1]  # Probability of being a 'snark'
...
create_table_query = '''
CREATE TABLE IF NOT EXISTS graph2p500000_random_forests_anomalies (
...
for index, row in df.iterrows():
    insert_query = '''
    INSERT INTO graph2p500000_random_forests_anomalies (number_of_vertices, smallest_eigenvalue, matching_number, 
                                      second_largest_eigenvalue, radius, density, girth, 
